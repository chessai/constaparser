  ...
  overrides = haskell.packages.${compiler}.override {
...

    {
      #mkDerivation = args: super.mkDerivation (args // {
        #doBenchmark = pkgs.lib.elem args.pname [ "constaparser" ]; 
        #doCheck = pkgs.lib.elem args.pname [ "constaparser" ]; 
        #doHaddock = false;
      #});
      
      constaparser = overrideCabal (build "constaparser" ./.) (drv: {
        doBenchmark = true;
        doCheck = true;
        doHaddock = false;
      });
...
in rec {
  ...
  constaparser = if pkgs.lib.inNixShell then drv.env else drv;
}
